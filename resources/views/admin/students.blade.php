                ...
                <tbody class="divide-y divide-gray-800">
                    @forelse($students as $student)
                        <tr class="hover:bg-gray-800/50 transition">
                            ...
                            <td class="px-6 py-4 text-gray-400">{{ $student->username }}</td>
                            <td class="px-6 py-4">
                                <span class="inline-block px-3 py-1 {{ $student->is_active ? 'bg-green-900/30 text-green-300' : 'bg-red-900/30 text-red-300' }} text-xs rounded-full font-semibold">
                                    {{ $student->is_active ? 'Active' : 'Inactive' }}
                                </span>
                            </td>
                            <td class="px-6 py-4 flex gap-2">
                                <a href="{{ route('admin.users.edit', $student) }}" class="text-blue-400 hover:text-blue-300 font-semibold text-sm">Edit</a>
                                <form action="{{ route('admin.users.delete', $student) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure?')">
                                    @csrf
                                    @method('DELETE')
